      ...
      <div class="flex justify-between h-16 py-3 px-4">
        ...
        <div class="flex items-center gap-4 h-full">
          <div class="flex items-center h-full text-slate-600">
            <button :class="[currentPage > 1 ? 'hover:bg-gray-50' : 'opacity-50']"
            ...
            <div class="-mx-px px-2 h-full flex items-center gap-2 border border-slate-300 bg-white">
              ...
              <input v-model="displayedPage"
                     name="page"
                     id="page"
                     type="number"
                     class="block w-7 h-7 rounded-md bg-slate-300 text-center focus:border-teal-500 focus:ring-teal-500"
                     placeholder="1"
                     @keyup.enter="emitUpdate" />
              <p>из {{ maxPage }}</p>
            </div>

            <button :class="[currentPage < maxPage ? 'hover:bg-gray-50' : 'opacity-50']"
